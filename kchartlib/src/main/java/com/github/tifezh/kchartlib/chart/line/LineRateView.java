...
public class LineRateView extends BaseLineRateView {
    ...
    public LineRateView(Context context) {
        super(context);
        initView();
    }

    public LineRateView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public LineRateView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void initData(Collection<? extends ILineRate> datas) {
        mPoints.clear();
        if (datas != null) {
    ...
    private void drawLine(Canvas canvas) {
        ...
        for (int i = mStartIndex; i <= mStopIndex; i++) {
            ILineRate currentPoint = getItem(i);
            float currentPointX = getX(i);
            ILineRate lastPoint = i == 0 ? currentPoint : getItem(i - 1);
        ...
        if (isLongPress || !isClosePress) {
            ILineRate point = getItem(mSelectedIndex);
            if (point == null) {
                return;
            }
...
//        canvas.drawCircle(stopX, getMainY(stopValue), 5 / mScaleX, mDotPaint);
    ...
    private void drawText(Canvas canvas, int position) {
        ...
        ILineRate point = getItem(position);
